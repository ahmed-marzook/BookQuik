...
SERVICES = {
    ...
    "apigateway": {
        "path": "./apigateway",
        "port": 8090,
        "jar_name": "apigateway-0.0.1-SNAPSHOT.jar",
    },
}
...
def build_service(service_name, skip_tests=False):
    ...
    if not run_command(
        f"docker build -t {image_name} -t {image_name_versioned} .", cwd=service_path
    ):
...
def main():
    ...
    parser.add_argument(
        "--skip-tests", action="store_true", help="Skip running tests during build"
    )
    parser.add_argument(
        "--push", action="store_true", help="Push images to Docker registry"
    )

    args = parser.parse_args()

    services_to_build = (
        list(SERVICES.keys()) if args.services == "all" else args.services
    )
